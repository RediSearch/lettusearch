...
[source,java]
----
StatefulRediSearchConnection<String, String> conn = RediSearchClient.create("redis://localhost").connect(); //<1>
RediSearchCommands<String, String> commands = connection.sync(); //<2>
Schema schema = Schema.builder().field(TextField.name("field1").build())...build(); //<3>
CreateOptions options = CreateOptions.builder()...build();
ft.create("beerIdx", schema, options); //<4>
Document<String, String> doc = Document.<String, String>builder().id("123").score(1D).build(); //<5>
doc.put("name", "La Chouffe");
ft.add("beerIdx", doc); //<6>
SearchResults<String, String> results = ft.search("beerIdx", "chou*"); //<7>
----
<1> Establish a connection to RediSearch
<2> Use _sync_, _async_, or _reactive_ API
<3> Create a schema
<4> Create an index with the schema
<5> Create a document
<6> Add document to the index
<7> Search the index
