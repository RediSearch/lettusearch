...
public class RediSearchReactiveCommandsImpl<K, V> extends RedisReactiveCommandsImpl<K, V>
	...
	@Override
	public Mono<String> add(K index, Document<K, V> document) {
	...
	@Override
	public Mono<String> create(K index, Schema<K> schema) {
		return createMono(() -> commandBuilder.create(index, schema));
	}

	@Override
	public Mono<String> create(K index, Schema<K> schema, CreateOptions<K, V> options) {
		return createMono(() -> commandBuilder.create(index, schema, options));
	}

	@Override
	public Mono<String> drop(K index) {
		return createMono(() -> commandBuilder.drop(index));
	}
	...
	@Override
	public Mono<Boolean> del(K index, K docId) {
		return createMono(() -> commandBuilder.del(index, docId));
	}
	...
	public Mono<SearchResults<K, V>> search(K index, V query) {
		return createMono(() -> commandBuilder.search(index, query));
	}
	...
	public Mono<AggregateResults<K, V>> aggregate(K index, V query) {
		return createMono(() -> commandBuilder.aggregate(index, query));
	}
	...
	public Mono<AggregateWithCursorResults<K, V>> aggregate(K index, V query, Cursor cursor) {
		return createMono(() -> commandBuilder.aggregate(index, query, cursor));
	}
	...
	@Override
	public Mono<AggregateWithCursorResults<K, V>> aggregate(K index, V query, Cursor cursor, Object... options) {
		return createMono(() -> commandBuilder.aggregate(index, query, cursor, options));
	}

	@Override
	public Mono<AggregateWithCursorResults<K, V>> cursorRead(K index, long cursor) {
		return createMono(() -> commandBuilder.cursorRead(index, cursor));
	}
